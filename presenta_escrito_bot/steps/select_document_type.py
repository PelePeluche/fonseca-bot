...
def select_document_type(driver, document_text,  document_type_text="EJECUCIÓN - PROCEDIMIENTO DE EJECUCIÓN FISCAL ADMINISTRATIVA"):
    """
    ...
    document_type_text = document_type_text if document_type_text else "OTRAS PETICIONES"

    # Seleccionar la opción 'OTRAS PETICIONES'
    add_option = WebDriverWait(driver, 40).until(
        EC.element_to_be_clickable((By.XPATH, f"//div[contains(text(), '{document_type_text}')]"))
    )
    add_option.click()
