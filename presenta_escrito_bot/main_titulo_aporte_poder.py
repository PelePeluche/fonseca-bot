...
def main(excel_path, docx_path, user_key):
    ...
    for index, row in df.iterrows():
        ...
        # Tomamos el valor de la columna "Planilla/PDF"
        planilla = row.get("Planilla/PDF")
        planilla_path = None
        if not pd.isnull(planilla):
            # Si no es nulo, construimos la ruta
            planilla_str = str(planilla)
            planilla_path = os.path.join(
                os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                'planillas',
                planillas_foldername,
                planilla_str,
            )

        print(f"Path para la planilla: {planilla_path if planilla_path else 'None'}")
...
if __name__ == "__main__":
    ...
    docx_path = os.path.join(base_path, "certificates", "PROVEA DEMANDA.docx")
    user_key = "SONZINI"
